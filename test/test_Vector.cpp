...
TEST_F(VectorTest, ConstructorTestCol) {
  ...
  ASSERT_EQ(x.on_grid(), (x.get_row_or_col() == "col")
                             ? (comm->get_row_color() == 0)
...
TEST_F(VectorTest, InitializeZeros) {
  ...
  if (y.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec, y.get_d_vec(),
                         (size_t)y.get_num_blocks() * y.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
  ...
  if (x.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec, x.get_d_vec(),
                         (size_t)x.get_num_blocks() * x.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
    size_t start = (x.get_row_or_col() == "col") ? comm->get_col_color()
                                                 : comm->get_row_color();
  ...
  if (y.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec, y.get_d_vec(),
                         (size_t)y.get_num_blocks() * y.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
    size_t start = (y.get_row_or_col() == "col") ? comm->get_col_color()
                                                 : comm->get_row_color();
...
TEST_F(VectorTest, DeepCopy) {
  ...
  if (x.on_grid()) {
    double *h_vec_1 =
        new double[(size_t)x.get_num_blocks() * x.get_block_size()];
...
TEST_F(VectorTest, Move) {
  ...
  if (xx.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec_1, xx.get_d_vec(),
                         (size_t)xx.get_num_blocks() * xx.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
  }
...
TEST_F(VectorTest, ScalarMult) {
  ...
  if (z2.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec, z2.get_d_vec(),
                         (size_t)z2.get_num_blocks() * z2.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
...
TEST_F(VectorTest, SetDVec) {
  ...
  double *d_vec;
  gpuErrchk(cudaMalloc(&d_vec, (size_t)x.get_num_blocks() * x.get_block_size() *
                                   sizeof(double)));
...
TEST_F(VectorTest, ReadFromFile) {
  std::string filename_param =
      dirname(__FILE__) + "/data/test_param_vec_SOTI.h5";
...
TEST_F(VectorTest, WriteToFile) {
  ...
  if (x.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec, x.get_d_vec(),
                         ...
                         cudaMemcpyDeviceToHost));
    double *h_vec2 =
        new double[(size_t)x2.get_num_blocks() * x2.get_block_size()];
    gpuErrchk(cudaMemcpy(h_vec2, x2.get_d_vec(),
                         (size_t)x2.get_num_blocks() * x2.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
    for (size_t i = 0; i < x.get_num_blocks() * x.get_block_size(); i++) {
  ...
  if (y.on_grid()) {
    ...
    gpuErrchk(cudaMemcpy(h_vec, y.get_d_vec(),
                         ...
                         cudaMemcpyDeviceToHost));
    double *h_vec2 =
        new double[(size_t)y2.get_num_blocks() * y2.get_block_size()];
    gpuErrchk(cudaMemcpy(h_vec2, y2.get_d_vec(),
                         (size_t)y2.get_num_blocks() * y2.get_block_size() *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));
    for (size_t i = 0; i < y.get_num_blocks() * y.get_block_size(); i++) {
      ASSERT_EQ(h_vec[i], h_vec2[i]);
...
TEST_F(VectorTest, Extend) {
  int current_bs = BLOCK_SIZE; // 4
  int new_bs = 10;             // Extend to 10

  // Test for 'col' vector
  Vector x = Vector(*comm, NUM_BLOCKS, current_bs, "col");
  x.init_vec_consecutive(); // Fill with 0, 1, 2, ...

  Vector y = x.extend(new_bs);

  // Verify Metadata
  ASSERT_EQ(y.get_block_size(), new_bs);
  ASSERT_EQ(y.get_num_blocks(), NUM_BLOCKS);

  // Verify Data
  if (x.on_grid()) {
    // Get original data to compare against
    double *h_x = new double[(size_t)x.get_num_blocks() * current_bs];
    gpuErrchk(
        cudaMemcpy(h_x, x.get_d_vec(),
                   (size_t)x.get_num_blocks() * current_bs * sizeof(double),
                   cudaMemcpyDeviceToHost));

    // Get new extended data
    double *h_y = new double[(size_t)y.get_num_blocks() * new_bs];
    gpuErrchk(cudaMemcpy(h_y, y.get_d_vec(),
                         (size_t)y.get_num_blocks() * new_bs * sizeof(double),
                         cudaMemcpyDeviceToHost));

    for (int b = 0; b < NUM_BLOCKS; ++b) {
      for (int i = 0; i < new_bs; ++i) {
        size_t idx_y = (size_t)b * new_bs + i;

        if (i < current_bs) {
          // Should match original data
          size_t idx_x = (size_t)b * current_bs + i;
          ASSERT_EQ(h_y[idx_y], h_x[idx_x])
              << "Mismatch at block " << b << " index " << i;
        } else {
          // Should be zero padded
          ASSERT_EQ(h_y[idx_y], 0.0)
              << "Padding failure at block " << b << " index " << i;
        }
      }
    }
    delete[] h_x;
...
TEST_F(VectorTest, Shrink) {
  int current_bs = BLOCK_SIZE; // 4
  int new_bs = 2;              // Shrink to 2

  // Test for 'col' vector
  Vector x = Vector(*comm, NUM_BLOCKS, current_bs, "col");
  x.init_vec_consecutive();

  Vector y = x.shrink(new_bs);
  ...
  if (x.on_grid()) {
    ...
    for (int b = 0; b < NUM_BLOCKS; ++b) {
      for (int i = 0; i < new_bs; ++i) {
        ...
        // The data in the shrunk vector should match the
        // first 'new_bs' elements of the original vector blocks
        ASSERT_EQ(h_y[idx_y], h_x[idx_x])
            << "Mismatch at block " << b << " index " << i;
      }
    }
...
TEST_F(VectorTest, ExtendAndShrinkRoundTrip) {
  // A quick consistency check: Extend -> Shrink back to original size
  // should result in the original vector (because we pad with zeros,
  // we don't destroy the original data structure, and shrinking removes the
  // zeros).

  int original_bs = BLOCK_SIZE;
  int large_bs = BLOCK_SIZE * 2;

  Vector x = Vector(*comm, NUM_BLOCKS, original_bs, "col");
  x.init_vec_consecutive();

  Vector extended = x.extend(large_bs);
  Vector restored = extended.shrink(original_bs);

  if (x.on_grid()) {
    double *h_x = new double[(size_t)x.get_num_blocks() * original_bs];
    gpuErrchk(
        cudaMemcpy(h_x, x.get_d_vec(),
                   (size_t)x.get_num_blocks() * original_bs * sizeof(double),
                   cudaMemcpyDeviceToHost));

    double *h_restored =
        new double[(size_t)restored.get_num_blocks() * original_bs];
    gpuErrchk(cudaMemcpy(h_restored, restored.get_d_vec(),
                         (size_t)restored.get_num_blocks() * original_bs *
                             sizeof(double),
                         cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < x.get_num_blocks() * original_bs; i++) {
      ASSERT_EQ(h_x[i], h_restored[i]);
    }
