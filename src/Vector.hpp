...
private:
  ...
  bool initialized =
      false;        /**< Flag indicating if the vector is initialized. */
  int checksum = 0; /**< Checksum for the vector. */

  // void to_TOSI_local(); /**< Reorder the vector to TOSI ordering (locally).
  // */ void to_SOTI_local(); /**< Reorder the vector to SOTI ordering
  // (locally). */
...
public:
   ...
   * @param blocks The number of blocks (local or global based on value of
   * global_sizes).
   ...
   */
  Vector(Comm &comm, unsigned int blocks, unsigned int block_size,
         std::string row_or_col, bool global_sizes = false,
  ...
  Vector(Vector &&vec) noexcept
      : comm(vec.comm), num_blocks(vec.num_blocks),
        ...
        block_size(vec.block_size), d_vec(vec.d_vec),
        row_or_col(vec.row_or_col), initialized(vec.initialized),
  ...
  /**
   * @brief Destructor for the Vector class. Frees the memory allocated for the
   * vector data.
   */
  ~Vector();
  ...
  /**
   * @brief Initializes the vector with double precision floating point numbers
   * (used for precision testing).
   */
   ...
   * @param QoI Flag indicating whether the vector is a quantity of interest or
   * regular observation (if applicable).
   */
  void init_vec_from_file(std::string filename, int checksum = 0,
                          bool QoI = false);
   ...
   * @param filename The name of the file.
   * @param QoI Flag indicating whether the vector is a quantity of interest
   * vector.
   */
