...
Vector::Vector(Comm &comm, unsigned int blocks, unsigned int block_size, std::string row_or_col,
    ...
    : comm(comm), block_size(block_size), padded_size(2 * block_size), row_or_col(row_or_col), SOTI_ordering(SOTI_ordering)
{
...
Vector::Vector(Vector &vec, bool deep_copy)
    : comm(vec.comm), num_blocks(vec.num_blocks), glob_num_blocks(vec.glob_num_blocks), padded_size(vec.padded_size), block_size(vec.block_size), row_or_col(vec.row_or_col), SOTI_ordering(vec.SOTI_ordering)
{
