 ...
 *
 * This file contains various definitions and includes that are commonly used
 * across multiple source files. It includes standard C and C++ libraries, CUDA
 * libraries, OpenMP, MPI, NCCL, and other necessary headers. It also defines
 * various constants, types, and helper functions for error checking.
 *
 * @note This file should be included in all source files that require these
 * shared definitions and includes.
 */
#ifndef __SHARED_H__
#define __SHARED_H__
...
public:
  ...
  T &operator[](int e) { return std::array<T, N>::operator[]((std::size_t)e); }
